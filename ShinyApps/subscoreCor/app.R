...
ui = fluidPage(
  # *Input() functions,
  fluidRow(h2("Total and Subscore Correlations", style="text-align: center;")),
  fluidRow(column(3,  sliderInput(inputId = "totalItem",
                      ...
                      sliderInput(inputId = "subItem",
                                label = "Number of Items in First Subscore",
                                value = 40, min=3, max=197), 
                      actionButton(inputId = "draw", label = "Resample Scores")
                  ),
           column(9,   # *Output() functions
                  plotOutput("corrMatrix"))),
  fluidRow(a("Link to R script underlying this Shiny App", href = "subscores_app_initial.R", target="_blank"))
